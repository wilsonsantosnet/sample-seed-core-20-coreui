...
{
	[Authorize]
    [Route("api/sampletag/more")]
    public class SampleTagMoreController : Controller
    {

        private readonly ISampleTagRepository _rep;
        private readonly ISampleTagApplicationService _app;
		private readonly ILogger _logger;
		private readonly EnviromentInfo _env;

        public SampleTagMoreController(ISampleTagRepository rep, ISampleTagApplicationService app, ILoggerFactory logger, EnviromentInfo env)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<SampleTagMoreController>();
			this._env = env;
        }
        ...
        public async Task<IActionResult> Get([FromQuery]SampleTagFilter filters)
        {
            ...
            try
            {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "SampleTag", this._env.RootPath);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<SampleTagDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Seed - SampleTag", dtos, new ErrorMapCustom());
            }

        }

		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<SampleTagDtoSpecialized> dtos)
        {
            var result = new HttpResult<SampleTagDto>(this._logger);
            try
            {
